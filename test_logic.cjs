...
const useTradeStore = createStore((set, get) => ({
  ...
  updateUnrealizedPnL: (currentPrice) => {
    ...
    console.log({ equity, positionValue, marginRequired, currentPrice });

    if (equity <= marginRequired) {
      console.log('ACCOUNT BLOWN TRIGGERED');
      flat(currentPrice);
      set({ isBlown: true });
      return;
    }
...
store.buy(60000);
console.log('After buy:', useTradeStore.getState().balance);
store.updateUnrealizedPnL(59958.14);
console.log('After update:', useTradeStore.getState().balance, useTradeStore.getState().isBlown);
