...
import { CircleOff, Wallet, BarChart3, Activity } from 'lucide-react';

export function TradingPanel() {
  const {
    balance, realizedPnL, unrealizedPnL, position, entryPrice, positionSize,
    buy, sell, flat, updateUnrealizedPnL, setPositionSize
  } = useTradeStore();
  ...
  }, [currentIndex, currentPrice, position, entryPrice, positionSize, updateUnrealizedPnL]);
  ...
  return (
    <div className="pt-4 border-t border-dark-700/50 space-y-4">
      <h4 className="text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">Trading Dashboard</h4>
      ...
      <div className="pt-3 border-t border-dark-700/30 space-y-1.5">
        ...
        <div className="flex justify-between items-center text-[10px] uppercase font-bold">
          ...
          <span className="text-slate-300 font-mono">{positionSize}</span>
        </div>
        <div className="flex justify-between items-center text-[10px] uppercase font-bold">
          <span className="text-slate-500">Unrealized P&L</span>
          <span className="text-slate-300 font-mono">{unrealizedPnL}</span>
        </div>
      </div>
    </div>
  );
